...
class Allocate(Coordinates):
    def __init__(
        ...
        self.subject = {
            i: [j[0] for j in DB.fetch(table=i)]  # unit of subject
            for i in {
                i[0] for i in DB.fetch(table="Professor", typ="specific", col="Subject")
            }  # subject set
        }
    ...
    def merge_data(self):
        ...
        for i in self.teacher:  # specifies required teacher for each unit in each classroom
            ...
            if len(current_subject) > len(current_teacher):
                TEACHERS_PER_UNIT = len(current_teacher)

            SECTIONS_PER_TEACHER = round(self.SECTIONS / TEACHERS_PER_UNIT)

            for j in range(
                round(len(current_teacher) / TEACHERS_PER_UNIT)
            ):  # filters teachers

                if (
                    len(current_teacher) >= TEACHERS_PER_UNIT
                ):  # Handeling population error (ValueError)

                    teacher = sample(current_teacher, k=TEACHERS_PER_UNIT)

                    # removing used teachers from current_teacher list
                    for choosen in teacher:
                        current_teacher.pop(current_teacher.index(choosen))

                    if i == 'Computer':
                        section = [self.SHIFT + str(i) for i in range(1, self.OPT + 1)]

                    elif i == 'Biology':
                        section = [
                            self.SHIFT + str(i + 1)
                            for i in range(self.OPT, self.SECTIONS) ]
                    ...
                    for k in range(self.SECTIONS):  # Distribute Sections
                        ...
                        if len(section) >= SECTIONS_PER_TEACHER and len(teacher) != len(
                            stored_section
                        ):
                            ...
                            for choosen in random_section:
                                section.pop(section.index(choosen))
                            ...
                            break  # End if section list is empty

                    try:
                        self.manage_data(
                            self.subject[i][j], teacher, stored_section
                            )  # sending data to sort function
