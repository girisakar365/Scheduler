'''
NOTE Main frame extraction is done from Lable.py so all the lables handled by 'Masters' in Lable.py must not be
included here.
'''
...
class Connect:
    ...
    def shortcut(self):
        ...
        bar = scroll_bar(MASTER,780,180,10,400)
        bar.valueChanged.connect( lambda: CHILD.move(120, 180 - (bar.value() + 5) ) )
        ...
        def lable():
            ...
            return record,professor,subject,time_table,user,guid
        ...
        def entry():
            ...
            for i in record,professor,subject,time_table,user,guid:
        ...
        general_btw=self.button_widget['shortcut_btw']
        general_btw.clicked.connect(lambda:self.frame_manager(MASTER))