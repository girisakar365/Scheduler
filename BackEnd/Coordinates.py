...
class Coordinates:
    ...
    def coordinates(self):  # manages coordinates
                ...
                if i != (self.DAYS + 1) * count:
                    ...
                    for j in range(self.PERIODS):  # columns
                        ...
                        if self.PERIODS >= min:
                            ...
                            if (i <= gap and j != max or i > gap and j != min):  
                            # manage breaks from data base remember to sort them
                                days.append((i, j))
        ...
        def lab_coordinates():
            ...
            checklist = [
                i for i in range(self.PERIODS) if i % 2 != 0
            ]  # check list / break list
                ...
                for j in self.row_col[self.SHIFT + str(i + 1)]:
                    try:
                        ...
                        if j[1] not in checklist:
                            lcord.append(
                                [j, self.row_col[self.SHIFT + str(i + 1)][
